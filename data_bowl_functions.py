"""Functions for the Duel Influence Probability (DIP) pipeline.

Loads weekly NFL tracking data, derives ball trajectories and player
movement, and computes the probability that a man-coverage defender or
the targeted receiver wins a thrown ball.
"""
...
def get_max_frame(all_situations: pd.DataFrame) -> None:
    ...
    Returns:
        None: Writes the result to max_frame.csv.
    """
    max_frame = (
...
def create_play_gif(
    ...
    Returns:
        None: Saves the animation to gifs\\play_<game_id>_<play_id>.gif.
    """
    # Prepare data
    ...
    def init() -> Tuple[plt.PathCollection, plt.PathCollection, plt.PathCollection]:
        """Reset the player, probability, and ball scatters to empty.

        Returns:
            Tuple[plt.PathCollection, plt.PathCollection,
            plt.PathCollection]: The cleared player, probability, and
    ...
    def update(
        frame,
    ) -> Tuple[plt.PathCollection, plt.PathCollection, plt.PathCollection]:
        ...
        Returns:
            Tuple[plt.PathCollection, plt.PathCollection,
            plt.PathCollection]: The updated player, probability, and
            ball scatter artists.
        """
